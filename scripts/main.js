...
function renderPage() {
   if (currentPage === 'pdf') {
       renderPdfImageList();
       // 💡 Call the new PDF size estimator when the PDF page renders
...
function handleFileSelect(event, mode) {
   ...
   if (mode === 'pdf') {
       ...
       Promise.all(newFilesPromises).then(() => {
           renderPdfImageList();
           // 💡 Update PDF size after adding new files
           updatePdfCompressionSize();
       });
...
function removeFile(id) {
   ...
   renderPdfImageList();
   // 💡 Update PDF size after removing a file
   updatePdfCompressionSize();
}
...
/**
* NEW LOGIC: Forces output to JPEG so the 'quality' parameter works.
* This is crucial for lossy compression and size reduction.
*/
function compressAndConvertToBase64(img, mimeType, quality) {
   ...
   // CRITICAL FIX: By forcing the output to 'image/jpeg', we ensure the
   // 'quality' parameter (0.1 to 1.0) is respected for lossy compression.
   return canvas.toDataURL('image/jpeg', quality);
}

// --- PDF Generation Implementation (MODIFIED) ---
async function processAndDownloadPDF(filename, applyGrayscale) {
   // 💡 Read quality from the new slider (divided by 100 for the 0.0 to 1.0 range)
   ...
   for (const fileObj of pdfFiles) {
       ...
       let dataURL;
       let format = 'JPEG'; // Set JPEG as default format for compression

       // 1. Image Processing & Format Determination
       if (applyGrayscale) {
           // grayscaleImage function already outputs JPEG at 0.9 quality
           dataURL = grayscaleImage(img);
       } else {
           // Standard conversion: Use the new function that forces JPEG conversion
           // with the user-defined 'quality' for effective size reduction.
       ...
       // 3. Add image to PDF using the determined format (now always JPEG for compression)
