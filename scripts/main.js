...
let isGrayscalePdfMode = false; // Used to track PDF type before modal confirmation
...
function clearHistory() {
    // Replaced confirm() with alertUser for compliance, but still requires manual confirmation logic
    if (getHistory().length === 0) return; // Prevent action if empty

    // In a production environment, this would be a custom modal confirmation.
    // For this context, we'll assume the user's intent is affirmative upon clicking.
...
window.clearHistory = clearHistory; // **FIX: Expose to global scope**
...
window.updatePdfCompressionSize = updatePdfCompressionSize; // **FIX: Expose to global scope**
...
window.removeFile = removeFile; // **FIX: Expose to global scope**
...
window.updateCompressionSize = updateCompressionSize; // **FIX: Expose to global scope**
...
function showNameModal(isGrayscale) {
    ...
    isGrayscalePdfMode = isGrayscale; // **FIX: Use local variable**
...
async function confirmDownload() {
    ...
    try {
        await processAndDownloadPDF(filename, isGrayscalePdfMode); // **FIX: Use local variable**
        
        // --- HISTORY LOGGING ---
        const quality = document.getElementById('pdf-quality-slider').value;
        const mode = isGrayscalePdfMode ? 'Grayscale' : 'Color';
...
async function processCompression() {
    ...
    try {
        ...
        // DataURL size approximation: 3 bytes of raw data = 4 characters of base64 data, so factor is ~ 0.75
        const compressedSize = compressedDataURL.length * 0.75 * 0.75; 
...
async function processAndDownloadPDF(filename, applyGrayscale) {
    ...
    // Ensure qualitySlider exists before reading its value
    const quality = qualitySlider ? (parseFloat(qualitySlider.value) / 100) : 0.8; 

    // Check if pdfFiles is empty, though confirmDownload should handle this, 
    // it's a good safety check for the async process.
    if (pdfFiles.length === 0) {
        throw new Error("No files selected for PDF generation.");
    }
    ...
    for (const fileObj of pdfFiles) {
        // This is where a corrupted file URL or a failed load could throw an error
        const img = await loadImage(fileObj.url);

        let dataURL;
        let format = 'JPEG'; // Always use JPEG for size reduction/compression
    ...
    // This is the final download command
